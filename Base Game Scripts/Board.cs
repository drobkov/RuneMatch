...
public class Board : MonoBehaviour
{
    ...
    private void GenerateConcreteTiles(){ //Генерация бетонных плиток
        //Look at all the tiles in the layout.
        for (int i = 0; i < boardLayout.Length; i++)
        {
    ...
    private void GenerateSlimeTiles(){ //Генерация slime плиток
        //Look at all the tiles in the layout.
        for (int i = 0; i < boardLayout.Length; i++)
        {
    ...
    private void SetUp(){
        GenerateBlankSpaces(); //Generate empty spaces on a board.
    ...
    private MatchType ColumnOrRow(){
        ...
        {
            //Store(хранить, складывать) this dot
            ...
            int columnMatch = 0;
            int rowMatch = 0;
            //Cycle through the rest of the pieces and compare(Сравнить)
            for (int j = 0; j < matchCopy.Count; j++)
            {
                //Store(хранить, складывать) the next dot
    ...
    private void CheckToMakeBombs(){
        ...
        {
            //What type of match?(Какой тип совпадений?)
    ...
    public void DestroyMatches(){
        ...
        //Сколько элементов содержится в списке совпадающих элементов из метода findmatches?
    ...
    private void RefillBoard(){
        // Логика для заполнения доски новыми фигурами
        for (int i = 0; i < width; i++){
            for (int j = 0; j < height; j++)
            {
    ...
    public IEnumerator ShuffleBoard(){
        //Перемешиваем фигуры если не осталось ходов.
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                //If this spot shouldn't be blank(empty)
                if (!blankSpaces[i, j] && !concreteTiles[i, j] && !slimeTiles[i, j])
                {
